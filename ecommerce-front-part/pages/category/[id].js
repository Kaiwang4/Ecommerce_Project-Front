...
export default function CategoryPage({category, subCategories, products: originalProducts}) {
    ...
    const defaultFilterValues = category.properties.map(property => ({name: property.name, value: 'all'}))
    ...
    function handleFilterChange(filterName, filterValue) {
        setFiltersValues(prev => {
            return prev.map(property => ({
                name: property.name,
                value: property.name === filterName ? filterValue : property.value
            }))
        })
    ...
    useEffect(() => {
        ...
        const categoryIds = [category._id, ...(subCategories?.map(subCategory => subCategory._id) || [])]
